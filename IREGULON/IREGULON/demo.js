...
//start of the function to search
var graphLoaded = false;

// Function to handle node selection after the graph is loaded
function handleNodeSelection(node) {
  // Show node information
  showNodeInfo(node);

  // Extract gene ID from selected node
  var geneId = node.id();

  // Ask for confirmation before sending the AJAX request
  $.ajax({
    type: "POST",
    url: "conn1.php",
    data: { geneInput: geneId },
    success: function (response) {
      // Handle success response if needed
      console.log(response);
      // Highlight the selected node after successful AJAX request
      highlight(node);
...
// Function to handle node selection before the graph is loaded
function handleNodeSelectionBeforeGraph(node) {
  Promise.resolve().then(function () {
    return highlight(node);
  });
}

// Click event handler for node selection
cy.on(
  "select unselect",
  "node",
  _.debounce(function (e) {
    ...
    if (node.nonempty()) {
      ...
      if (graphLoaded) {
        ...
        handleNodeSelectionBeforeGraph(node);
      }
    } else {
      // Clear node info and selection
      hideNodeInfo();
      clear();
    }
  }, 100)
...
// When the graph is loaded, set graphLoaded flag to true
